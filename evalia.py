...
def extract_reasoning_map(text: str) -> dict:
    ...
    Parse ONLY the 'Reasoning per category:' section into a dict:
      { 'Logic': '<reasoning text>', ... }
    """
    reasoning_start = text.find("🧪 Reasoning per category:")
    ...
    for i, cat in enumerate(categories):
        start = reasoning_text.find(f"{cat}:")
        end = reasoning_text.find(categories[i+1] + ":") if i+1 < len(categories) else None
        if start != -1:
            snippet = reasoning_text[start + len(cat) + 1 : end].strip()
            reasoning_map[cat] = snippet
...
def render_evalia_seal(verdict_text: str, brutality_mode: bool, logo_path: str = None) -> bytes:
    ...
    # Footer
    draw.text((W // 2, H - 10), "Evalia", font=small_font, fill=accent_text, anchor="mm")
